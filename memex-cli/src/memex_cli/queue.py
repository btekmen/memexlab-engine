...
def _load(vault: Vault, rel: pathlib.Path) -> tuple[dict, str]:
    meta, body = _parse_frontmatter((vault.root / rel).read_text(encoding="utf-8"))
...
def drop_queue_item(vault: Vault, item: str, agent: str = "memex-cli") -> dict:
    """Cancel a queue item without completing it (set status to cancelled)."""
    rel = _item_path(vault, item)
    meta, body = _load(vault, rel)
    st = str(meta.get("status", "pending"))
    if st not in COMPLETABLE:
        raise ValueError(f"queue item '{rel.stem}' is '{st}' — only {' / '.join(COMPLETABLE)} can be cancelled")
    
    now = datetime.datetime.now(datetime.timezone.utc)
    meta.update({"status": "cancelled", "cancelled_at": now.isoformat(), "cancelled_by": agent})
    (vault.root / rel).write_text(
    ...
    with log_path.open("a", encoding="utf-8") as f:
        f.write(json.dumps({
            "ts": now.isoformat(), "agent": agent, "action": "drop_queue_item",
            "path": str(rel),
        }, ensure_ascii=False) + "\n")
    return {"item": rel.stem, "path": str(rel), "status": "cancelled", "logged": True}
