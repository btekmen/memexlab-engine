...
*Until the first PyPI release lands, use:* `claude mcp add memexlab -- uvx --from "git+https://github.com/btekmen/memexlab-engine#subdirectory=memexlab-mcp" memexlab-mcp --vault ~/vault`
...
- **`memex-cli/`** — the **`memex` CLI, built from scratch, ingest spine first**: `memex ingest url` captures a public page into `inbox/` as readable markdown with provenance (dry-run default, idempotent, no LLM). Try it: `uvx --from "git+https://github.com/btekmen/memexlab-engine#subdirectory=memex-cli" memex --help`. See [`memex-cli/README.md`](memex-cli/README.md).
